...
*
* @file UDP_CLIENT.c
* @brief udp Client process which does socket udp communication with relaible transfer
* Reliable stop and wait protocol is being implemented for the communication
*
* @author  Gautham K A
* @date  9/21/2018
*
...
void put_file(char* file)
{
  ...
  //packet pointer
	...
	// timer of 500 ms for socket receive timeout for reliability transfer
	timeout.tv_sec = 0;
	...
	while(siz>0){
	  ...
	  printf("Client reading Bytes: %d \n",n);
		...
		packet->ack=1;
		ack=1;
		// ack confirm loop : loop until ack is received
		while(ack<=1){
					pkt->ack=0;pkt->seq=0;
...
********************************************************************************************************/
// this function is similar to put file in server
